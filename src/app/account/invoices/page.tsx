...
export default function AccountInvoicesPage() {
  ...
  return (
    <div className="rounded-2xl border border-border bg-white p-6 md:p-8">
      <h2 className="font-display text-xl font-semibold text-deep-blue">ใบแจ้งหนี้</h2>
      <p className="mt-1 text-sm text-muted">เอกสารเรียกเก็บเงินของบัญชีคุณ</p>

      {loading && <p className="mt-6 text-sm text-muted">กำลังโหลด...</p>}
      {error && <p className="mt-6 text-sm text-danger">{error}</p>}
      {!loading && !error && rows.length === 0 && (
        <p className="mt-6 text-sm text-muted">ยังไม่มีใบแจ้งหนี้</p>
      )}
          ...
          <table className="w-full min-w-[480px] text-left text-sm">
            <thead>
              <tr className="border-b border-border text-muted">
                ...
                <th className="pb-3 font-medium">ครบกำหนด</th>
            ...
            <tbody className="divide-y divide-border">
              {rows.map((row) => (
                <tr key={row.id}>
                  ...
                  <td className="py-3 text-muted">{row.due_date ?? "—"}</td>
                  <td className="py-3">
