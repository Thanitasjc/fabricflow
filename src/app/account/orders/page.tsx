...
export default function AccountOrdersPage() {
  ...
  useEffect(() => {
    authApi
      .orders()
      .then((data) => setRows(data as OrderRow[]))
  ...
  return (
    <div className="rounded-2xl border border-border bg-white p-6 md:p-8">
      <h2 className="font-display text-xl font-semibold text-deep-blue">คำสั่งซื้อ</h2>
      <p className="mt-1 text-sm text-muted">ประวัติและสถานะออเดอร์ของคุณ</p>

      {loading && <p className="mt-6 text-sm text-muted">กำลังโหลด...</p>}
      {error && <p className="mt-6 text-sm text-danger">{error}</p>}
      {!loading && !error && rows.length === 0 && (
        <p className="mt-6 text-sm text-muted">ยังไม่มีคำสั่งซื้อ</p>
      )}
            ...
            <tbody className="divide-y divide-border">
              {rows.map((row) => (
                <tr key={row.id}>
                  ...
                  <td className="py-3 text-muted">{row.order_date ?? "—"}</td>
                  <td className="py-3">
